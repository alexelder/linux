					  ...
					  struct dc_hw_fb *fb, struct drm_plane_state *state)
{
    ...
    sifive_ccache_flush_range(fb->y_address, fb->height * fb->y_stride);
    if (fb->u_address)
            sifive_ccache_flush_range(fb->u_address, fb->height * fb->u_stride);
    if (fb->v_address)
            sifive_ccache_flush_range(fb->v_address, fb->height * fb->v_stride);
