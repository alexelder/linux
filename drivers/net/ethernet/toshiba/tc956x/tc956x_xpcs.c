...
int stm_xpcs_init(struct stmmac_priv *priv, void __iomem *xpcsaddr)
{
	...
	reg_value = stm_xpcs_read(xpcsaddr, XGMAC_SR_MII_CTRL);
	...
	if (priv->wol_config_enabled != true) {
		...
		if (priv->plat->interface == PHY_INTERFACE_MODE_SGMII) {
			...
			reg_value = stm_xpcs_read(xpcsaddr, XGMAC_SR_XS_PCS_CTRL2);
			reg_value &= XGMAC_PCS_TYPE_SEL;
			reg_value |= 0x1;
			stm_xpcs_write(xpcsaddr, XGMAC_SR_XS_PCS_CTRL2, reg_value);

			reg_value = stm_xpcs_read(xpcsaddr, XGMAC_VR_MII_AN_CTRL);
			reg_value &= XGMAC_PCS_MODE_MASK;
			reg_value |= XGMAC_SGMII_MODE;/*SGMII PCS MODE*/
			stm_xpcs_write(xpcsaddr, XGMAC_VR_MII_AN_CTRL, reg_value);

			if (priv->is_sgmii_2p5g == true) {
				reg_value = stm_xpcs_read(xpcsaddr, XGMAC_VR_XS_PCS_DIG_CTRL1);
				reg_value &= ~(0x4);
				/* Enable only if SGMII 2.5G is enabled */
				reg_value |= 0x4; /*EN_2_5G_MODE*/
				stm_xpcs_write(xpcsaddr, XGMAC_VR_XS_PCS_DIG_CTRL1, reg_value);
			}
		}
		if ((priv->plat->interface == PHY_INTERFACE_MODE_USXGMII) ||
			...
			reg_value = stm_xpcs_read(xpcsaddr, XGMAC_VR_XS_PCS_KR_CTRL);
			...
			stm_xpcs_write(xpcsaddr, XGMAC_VR_XS_PCS_KR_CTRL, reg_value);
...
#endif /* #ifdef CONFIG_TC956X_MAGIC_PACKET_WOL_CONF */
#ifdef EEE
	reg_value = stm_xpcs_read(xpcsaddr, XGMAC_SR_XS_PCS_CTRL1);
	reg_value |= XGMAC_LPI_ENABLE;/* LPM : power down */
	stm_xpcs_write(xpcsaddr, XGMAC_SR_XS_PCS_CTRL1, reg_value);

	reg_value = stm_xpcs_read(xpcsaddr, XGMAC_VR_XS_PCS_DIG_STS);
	reg_value &= ~(XGMAC_PSEQ_STATE);/* PSEQ_STATE(B4:2)=3'b000 */
	stm_xpcs_write(xpcsaddr, XGMAC_VR_XS_PCS_DIG_STS, reg_value);

	reg_value = stm_xpcs_read(xpcsaddr, XGMAC_SR_XS_PCS_CTRL1);
	reg_value &= ~(XGMAC_LPI_ENABLE);/* LPM : Normal Operation */
	stm_xpcs_write(xpcsaddr, XGMAC_SR_XS_PCS_CTRL1, reg_value);

	reg_value = stm_xpcs_read(xpcsaddr, XGMAC_SR_XS_PCS_EEE_ABL);
	reg_value |= XGMAC_KXEEE;/* KXEEE */
	stm_xpcs_write(xpcsaddr, XGMAC_SR_XS_PCS_EEE_ABL, reg_value);

	reg_value = stm_xpcs_read(xpcsaddr, XGMAC_VR_XS_PCS_EEE_MCTRL0);
	...
	stm_xpcs_write(xpcsaddr, XGMAC_VR_XS_PCS_EEE_MCTRL0, reg_value);

	reg_value = stm_xpcs_read(xpcsaddr, XGMAC_VR_XS_PCS_EEE_TXTIMER);
...
#endif
	stm_xpcs_write(xpcsaddr, XGMAC_VR_XS_PCS_EEE_TXTIMER, reg_value);

	reg_value = stm_xpcs_read(xpcsaddr, XGMAC_VR_XS_PCS_EEE_RXTIMER);
...
#endif
	stm_xpcs_write(xpcsaddr, XGMAC_VR_XS_PCS_EEE_RXTIMER, reg_value);

	reg_value = stm_xpcs_read(xpcsaddr, XGMAC_VR_XS_PCS_EEE_MCTRL1);
	reg_value |= XGMAC_TRN_LPI;
	stm_xpcs_write(xpcsaddr, XGMAC_VR_XS_PCS_EEE_MCTRL1, reg_value);
...
void stm_xpcs_ctrl_ane(struct stmmac_priv *priv, bool ane)
{
	...
	reg_value = stm_xpcs_read(priv->xpcsaddr, XGMAC_SR_MII_CTRL);
#ifdef CONFIG_TC956X_MAGIC_PACKET_WOL_CONF
	if (priv->wol_config_enabled != true) {
#endif /* #ifdef CONFIG_TC956X_MAGIC_PACKET_WOL_CONF */
		if (ane) {
...
#endif /* #ifdef CONFIG_TC956X_MAGIC_PACKET_WOL_CONF */
	stm_xpcs_write(priv->xpcsaddr, XGMAC_SR_MII_CTRL, reg_value);
}

/**
 *  stm_xpcs_ctrl0_lrx - to configure XPCS LPI Rx Enable bit
...
void stm_xpcs_ctrl0_lrx(struct stmmac_priv *priv, bool lrx)
{
	...
	reg_value = stm_xpcs_read(priv->xpcsaddr, XGMAC_VR_XS_PCS_EEE_MCTRL0);
	if (lrx) {
	...
	stm_xpcs_write(priv->xpcsaddr, XGMAC_VR_XS_PCS_EEE_MCTRL0, reg_value);
}
#endif
