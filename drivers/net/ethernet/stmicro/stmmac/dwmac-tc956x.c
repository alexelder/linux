...
#define TC956X_RX_FIFO_SIZE		(46 * SZ_1K)
#define TC956X_TX_FIFO_SIZE		(46 * SZ_1K)
...
static const char * const tc956x_reset_names[] = {
...
static void tc956x_mac_enable(struct tc956x_data *td)
{
	...
	if (td->plat->phy_interface == PHY_INTERFACE_MODE_SGMII)
		tc956x_mac_configure(td, PHY_INTERFACE_MODE_SGMII, SPEED_100);
	tc956x_pma_init(td);
...
static int tc956x_probe_config_dt(struct tc956x_data *td)
{
	...
	if (plat->rx_fifo_size > TC956X_RX_FIFO_SIZE ||
	    plat->tx_fifo_size > TC956X_TX_FIFO_SIZE)
	 ...
	 * generic drivers cannot possibly work on TC956x (any without clocks
