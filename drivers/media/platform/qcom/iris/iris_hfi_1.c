...
static u32 iris_hfi_1_buf_type_from_driver(enum iris_buffer_type buffer_type)
{
	switch (buffer_type) {
	case BUF_INPUT:
...
static int iris_hfi_1_sys_init(struct iris_core *core)
{
...
static int iris_hfi_1_sys_image_version(struct iris_core *core)
{
...
static int iris_hfi_1_sys_interframe_powercollapse(struct iris_core *core)
{
...
static int iris_hfi_1_sys_pc_prep(struct iris_core *core)
{
...
static int iris_hfi_1_session_open(struct iris_inst *inst, u32 codec)
{
...
static void iris_hfi_1_packet_session_cmd(struct iris_inst *inst,
...
static int iris_hfi_1_session_close(struct iris_inst *inst)
{
	struct hfi_session_pkt packet;
	int ret;

	iris_hfi_1_packet_session_cmd(inst, &packet, HFI_CMD_SYS_SESSION_END);
...
static int iris_hfi_1_session_start(struct iris_inst *inst, u32 plane)
{
	struct hfi_session_pkt packet;
	...
	reinit_completion(&inst->completion);
	iris_hfi_1_packet_session_cmd(inst, &packet, HFI_CMD_SESSION_LOAD_RESOURCES);
	...
	reinit_completion(&inst->completion);
	iris_hfi_1_packet_session_cmd(inst, &packet, HFI_CMD_SESSION_START);
...
static int iris_hfi_1_session_stop(struct iris_inst *inst, u32 plane)
{
	...
	if ((V4L2_TYPE_IS_OUTPUT(plane) &&
		...
		reinit_completion(&inst->completion);
		iris_hfi_1_packet_session_cmd(inst, &packet, HFI_CMD_SESSION_STOP);
		ret = iris_hfi_queue_cmd_write(core, &packet, packet.shdr.hdr.size);
		if (ret)
			return ret;
		...
		reinit_completion(&inst->completion);
		iris_hfi_1_packet_session_cmd(inst, &packet, HFI_CMD_SESSION_RELEASE_RESOURCES);
		ret = iris_hfi_queue_cmd_write(core, &packet, packet.shdr.hdr.size);
		if (ret)
			return ret;
		...
		return iris_inst_change_sub_state(inst, IRIS_INST_SUB_LOAD_RESOURCES, 0);
	}
	...
	pkt.shdr.hdr.size = sizeof(struct hfi_session_flush_pkt);
	pkt.shdr.hdr.pkt_type = HFI_CMD_SESSION_FLUSH;
	pkt.shdr.session_id = inst->session_id;
	pkt.flush_type = flush_type;

	ret = iris_hfi_queue_cmd_write(core, &pkt, pkt.shdr.hdr.size);
	if (ret)
		return ret;

	return iris_wait_for_session_response(inst, true);
}

static int iris_hfi_1_session_continue(struct iris_inst *inst, u32 plane, u32 payload)
{
	struct hfi_session_pkt packet;
	int ret;

	if (!V4L2_TYPE_IS_OUTPUT(plane) && payload != HFI_CMD_SETTINGS_CHANGE)
		return 0;

	iris_hfi_1_packet_session_cmd(inst, &packet, HFI_CMD_SESSION_CONTINUE);
...
static int iris_hfi_1_session_queue_buffer(struct iris_inst *inst, struct iris_buffer *buf)
{
	...
	if (buf->type == BUF_INPUT) {
		...
		ret = iris_hfi_queue_cmd_write(inst->core, &ip_pkt, ip_pkt.shdr.hdr.size);
		...
		ret = iris_hfi_queue_cmd_write(inst->core, &op_pkt, op_pkt.shdr.hdr.size);
	} else if (buf->type == BUF_DPB) {
		...
		int_pkt = (struct hfi_session_set_buffers_pkt *)packet;

		buffer_type = iris_hfi_1_buf_type_from_driver(buf->type);
		...
		int_pkt->shdr.hdr.size = sizeof(*int_pkt) +
			((int_pkt->num_buffers) * sizeof(u32));
...
static int iris_hfi_1_session_unset_buffers(struct iris_inst *inst, struct iris_buffer *buf)
{
	...
	if (buffer_type == HFI_BUFFER_OUTPUT ||
		...
		pkt->shdr.hdr.size =
				...
				sizeof(u32) + (pkt->num_buffers * sizeof(*bi));
	} else {
		for (i = 0; i < pkt->num_buffers; i++)
...
static int iris_hfi_1_session_drain(struct iris_inst *inst, u32 plane)
{
...
static int
iris_hfi_1_packet_session_set_property(struct hfi_session_set_property_pkt *packet,
				       struct iris_inst *inst, u32 ptype, void *pdata)
{
	void *prop_data;

	if (!packet || !inst || !pdata)
		return -EINVAL;
	...
	case HFI_PROPERTY_PARAM_VDEC_OUTPUT_ORDER: {
		packet->data[1] = HFI_OUTPUT_ORDER_DECODE;
	...
	case HFI_PROPERTY_PARAM_VDEC_CONCEAL_COLOR: {
		...
		packet->shdr.hdr.size += sizeof(u32) + sizeof(*color);
		break;
	}
	default:
		return -EINVAL;
...
static int hfi_1_set_property(struct iris_inst *inst, u32 packet_type, void *payload)
{
	...
	ret = iris_hfi_1_packet_session_set_property(pkt, inst, packet_type, payload);
	if (ret == -EOPNOTSUPP)
		return 0;
	if (ret)
		return ret;
...
static int iris_hfi_1_session_set_property(struct iris_inst *inst, u32 packet_type,
					   u32 flag, u32 plane, u32 payload_type,
					   void *payload, u32 payload_size)
{
	return hfi_1_set_property(inst, packet_type, payload);
}

static int iris_hfi_1_set_resolution(struct iris_inst *inst)
{
	...
	ret = hfi_1_set_property(inst, ptype, &fs);
	if (ret)
		return ret;
...
static int iris_hfi_1_decide_core(struct iris_inst *inst)
{
	...
	return hfi_1_set_property(inst, ptype, &cu);
}

static int iris_hfi_1_set_raw_format(struct iris_inst *inst)
{
	...
	if (iris_split_mode_enabled(inst)) {
		...
		ret = hfi_1_set_property(inst, ptype, &fmt);
		if (ret)
			return ret;
...
static int iris_hfi_1_set_format_constraints(struct iris_inst *inst)
{
	...
	return hfi_1_set_property(inst, ptype, &pconstraint);
}

static int iris_hfi_1_set_num_bufs(struct iris_inst *inst)
{
	...
	buf_count.count_actual = inst->buffers.input.actual_count;

	ret = hfi_1_set_property(inst, ptype, &buf_count);
	if (ret)
		return ret;

	if (iris_split_mode_enabled(inst)) {
		buf_count.type = HFI_BUFFER_OUTPUT;
		buf_count.count_actual = inst->buffers.output.actual_count;
...
static int iris_hfi_1_set_multistream(struct iris_inst *inst)
{
	...
	if (iris_split_mode_enabled(inst)) {
		...
		ret = hfi_1_set_property(inst, ptype, &multi);
		if (ret)
			return ret;
...
static int iris_hfi_1_set_bufsize(struct iris_inst *inst)
{
	...
	if (iris_split_mode_enabled(inst)) {
		...
		ret = hfi_1_set_property(inst, ptype, &bufsz);
		if (ret)
			return ret;

		bufsz.type = HFI_BUFFER_OUTPUT2;
		bufsz.size = inst->buffers.output.size;
...
static int iris_hfi_1_session_set_config_params(struct iris_inst *inst, u32 plane)
{
	...
	static const struct iris_hfi_prop_type_handle prop_type_handle_inp_arr[] = {
		{HFI_PROPERTY_PARAM_FRAME_SIZE,
			iris_hfi_1_set_resolution},
		{HFI_PROPERTY_CONFIG_VIDEOCORES_USAGE,
			iris_hfi_1_decide_core},
		{HFI_PROPERTY_PARAM_UNCOMPRESSED_FORMAT_SELECT,
			iris_hfi_1_set_raw_format},
		{HFI_PROPERTY_PARAM_UNCOMPRESSED_PLANE_ACTUAL_CONSTRAINTS_INFO,
			iris_hfi_1_set_format_constraints},
		{HFI_PROPERTY_PARAM_BUFFER_COUNT_ACTUAL,
			iris_hfi_1_set_num_bufs},
		{HFI_PROPERTY_PARAM_VDEC_MULTI_STREAM,
			iris_hfi_1_set_multistream},
		{HFI_PROPERTY_PARAM_BUFFER_SIZE_ACTUAL,
			iris_hfi_1_set_bufsize},
	};
...
static const struct iris_hfi_ops iris_hfi_1_ops = {
	.sys_init = iris_hfi_1_sys_init,
	.sys_image_version = iris_hfi_1_sys_image_version,
	.sys_pc_prep = iris_hfi_1_sys_pc_prep,
	.sys_interframe_powercollapse = iris_hfi_1_sys_interframe_powercollapse,
	.session_open = iris_hfi_1_session_open,
	.session_set_config_params = iris_hfi_1_session_set_config_params,
	.session_set_property = iris_hfi_1_session_set_property,
	.session_start = iris_hfi_1_session_start,
	.session_queue_buf = iris_hfi_1_session_queue_buffer,
	.session_release_buf = iris_hfi_1_session_unset_buffers,
	.session_pause = NULL,
	.session_resume = iris_hfi_1_session_continue,
	.session_stop = iris_hfi_1_session_stop,
	.session_drain = iris_hfi_1_session_drain,
	.session_close = iris_hfi_1_session_close,
};

void iris_hfi_1_ops_init(struct iris_core *core)
{
	core->hfi_ops = &iris_hfi_1_ops;
}
