...
static void iris_hfi_queue_set_hdr_defaults(struct iris_hfi_queue_header *q_hdr)
{
...
int iris_hfi_queue_cmd_write(struct iris_core *core, void *pkt, u32 pkt_size)
{
	...
	return pm_runtime_put_autosuspend(core->dev);
...
int iris_hfi_queue_msg_read(struct iris_core *core, void *pkt)
{
	...
	if (!q_info || !q_info->q_array.kernel_vaddr || !pkt) {
		dev_err(core->dev, "cannot read from shared message queue\n");
		ret = -ENODATA;
		goto unlock;
	}
...
{
	iris_hfi_queue_set_hdr_defaults(iface_q->qhdr);
...
int iris_hfi_queues_init(struct iris_core *core)
{
	...
	if (core->iface_q_table.kernel_vaddr) {
		iris_hfi_queue_set_header(core, IFACEQ_CMDQ_ID, &core->command_queue);
		iris_hfi_queue_set_header(core, IFACEQ_MSGQ_ID, &core->message_queue);
		iris_hfi_queue_set_header(core, IFACEQ_DBGQ_ID, &core->debug_queue);
		return 0;
	}
	...
	q_tbl_hdr->qtbl_qhdr_size = sizeof(struct iris_hfi_queue_header);
