...
int iris_vdec_inst_init(struct iris_inst *inst)
{
	...
	inst->buffers.input.min_count = iris_vpu_buf_count(inst, BUF_INPUT);
...
int iris_vdec_s_fmt(struct iris_inst *inst, struct v4l2_format *f)
{
	...
	if (V4L2_TYPE_IS_OUTPUT(f->type)) {
		...
		if (!q->streaming)
			inst->buffers.output.min_count = iris_vpu_buf_count(inst, BUF_INPUT);
		if (inst->buffers.output.actual_count < inst->buffers.output.min_count)
			inst->buffers.output.actual_count = inst->buffers.output.min_count;

		inst->buffers.output.size = fmt->fmt.pix_mp.plane_fmt[0].sizeimage;
...
int iris_vdec_session_streamoff(struct iris_inst *inst, u32 plane)
{
	...
	ret = core->hfi_ops->session_stop(inst, plane);
	if (ret)
		goto error;
...
static int iris_vdec_process_streamon_input(struct iris_inst *inst)
{
	...
	ret = core->hfi_ops->session_start(inst, V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE);
	if (ret)
		return ret;
	...
	if (inst->sub_state & IRIS_INST_SUB_DRC ||
		...
		if (!(inst->sub_state & IRIS_INST_SUB_INPUT_PAUSE)) {
			ret = core->hfi_ops->session_pause(inst,
...
int iris_vdec_streamon_input(struct iris_inst *inst)
{
	...
	// TODO: VN: need to ensure only to call this once per session.
...
static int iris_vdec_process_streamon_output(struct iris_inst *inst)
{
	...
	if (inst->state == IRIS_INST_INPUT_STREAMING &&
		...
		if (!drain_active) {
			ret = core->hfi_ops->session_resume(inst,
							    V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE,
	...
	ret = core->hfi_ops->session_start(inst, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
	if (ret)
		return ret;
...
int iris_vdec_streamon_output(struct iris_inst *inst)
{
	...
	core = inst->core;
	ret = core->hfi_ops->session_set_config_params(inst, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
	if (ret)
		return ret;

	ret = iris_get_internal_buffers(inst, V4L2_BUF_TYPE_VIDEO_CAPTURE_MPLANE);
	if (ret)
		goto error;
...
int iris_vdec_qbuf(struct iris_inst *inst, struct vb2_v4l2_buffer *vbuf)
{
	...
	if (!iris_allow_qbuf(inst, vb2->type)) {
		dev_err(inst->core->dev, "qbuf not allowed\n");
	...
	dev_dbg(inst->core->dev, "queue buffer ret %d", ret);
	return ret;
}
...
int iris_vdec_stop_cmd(struct iris_inst *inst)
{
	...
	core = inst->core;
	ret = core->hfi_ops->session_drain(inst, V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE);
	if (ret)
		return ret;
