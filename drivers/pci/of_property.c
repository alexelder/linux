 ...
 * node "ranges" property.  The total number of cells in the array
...
dev_info(&pdev->dev, " === %s device first %u num %u %u resources\n",
	bridge_device ? "bridge" : "endpoint", first, num, resource_count);
...
dev_info(&pdev->dev, " === 0x%x 0x%x 0x%x  0x%x 0x%x 0x%x  0x%x 0x%x\n",
	ep->child_addr[0], ep->child_addr[1], ep->child_addr[2],
	ep->parent_addr[0], ep->parent_addr[1], ep->parent_addr[2],
	ep->size[0], ep->size[1]);
